        ...
        <div class="bg-white border border-[#e5e5e5] rounded-xl p-6">
            <form action="{{ route('stock-transfers.store') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="3" placeholder="Add any additional notes or instructions..." class="w-full px-4 py-2.5 text-sm bg-white border border-[#e5e5e5] rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-gray-900 resize-none"></textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
