...
<head>
    ...
    <title>Import Results - Transactions</title>
    ...
    <div class="min-h-screen flex flex-col">
        ...
        <div class="bg-white border-b border-[#e5e5e5]">
            <div class="max-w-7xl mx-auto px-6 py-6">
                <h1 class="text-2xl font-semibold text-gray-900">Import Results</h1>
                <p class="mt-1 text-sm text-gray-500">Batch upload processing results</p>
            </div>
        </div>
        ...
        <main class="flex-1 max-w-7xl mx-auto px-6 py-6 w-full">
            ...
            <div class="bg-white border border-[#e5e5e5] rounded-xl mb-6">
                <div class="px-6 py-4 border-b border-[#e5e5e5]">
                    <h2 class="text-lg font-medium text-gray-900">Import Summary</h2>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Total Records</label>
                            <p class="text-2xl font-semibold text-gray-900">{{ $results['total'] ?? 0 }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Successful</label>
                            <p class="text-2xl font-semibold text-green-700">{{ $results['successful'] ?? 0 }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Failed</label>
                            <p class="text-2xl font-semibold text-red-700">{{ $results['failed'] ?? 0 }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500 mb-1">Skipped</label>
            ...
            <div class="bg-white border border-[#e5e5e5] rounded-xl mb-6">
                <div class="px-6 py-4 border-b border-[#e5e5e5]">
                    <h2 class="text-lg font-medium text-gray-900">Successfully Imported</h2>
                </div>
                        ...
                        <tbody class="divide-y divide-[#e5e5e5]">
                            @foreach($results['successful_records'] as $record)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $record['row'] ?? 'N/A' }}</td>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $record['amount'] ?? 'N/A' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $record['currency'] ?? 'N/A' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $record['transaction_id'] ?? 'N/A' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
