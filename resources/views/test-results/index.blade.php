            ...
            <div class="bg-surface border border-border rounded-xl p-5">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-2xl font-semibold text-ink">{{ $statistics['pass_rate'] }}%</p>
                    </div>
        ...
        <div class="bg-surface border border-border rounded-xl p-4">
            <form method="GET" action="{{ route('test-results.index') }}" class="flex flex-wrap gap-4 items-end">
                <div class="flex-1 min-w-[200px]">
                    <label for="status" class="block text-sm font-medium text-ink-muted mb-1">Status</label>
                ...
                <div class="flex-1 min-w-[200px]">
                    <label for="suite" class="block text-sm font-medium text-ink-muted mb-1">Test Suite</label>
        ...
        <div class="bg-surface border border-border rounded-xl overflow-hidden">
            <table class="min-w-full divide-y divide-border">
                <thead class="bg-canvas-subtle">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-ink-muted uppercase tracking-wider">Run ID</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-ink-muted uppercase tracking-wider">Duration</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-ink-muted uppercase tracking-wider">Date</th>
                    ...
                    @forelse($testRuns as $run)
                        <tr class="hover:bg-canvas-subtle">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex px-2.5 py-0.5 text-xs font-medium rounded bg-canvas-subtle text-ink-muted">{{ ucfirst($run->status) }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <a href="{{ route('test-results.show', $run->id) }}"
                                   class="text-ink hover:text-ink-muted font-medium">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-sm text-ink-muted">No test runs found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
