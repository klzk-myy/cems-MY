        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-500">FIND-{{ $finding->id }}</p>
                </div>
                <a href="{{ route('compliance.findings.index') }}" class="px-4 py-2 text-sm font-medium rounded-lg bg-white border border-[#e5e5e5] hover:bg-gray-50">
                    Back to List
                </a>
            </div>
        </div>
        ...
        <div class="bg-white border border-[#e5e5e5] rounded-xl p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $finding->finding_type?->label() ?? '—' }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $finding->category?->label() ?? $finding->category ?? 'Compliance' }}</p>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 uppercase mb-1">Generated At</label>
                    <p class="text-sm text-gray-900">{{ $finding->generated_at?->format('Y-m-d H:i') ?? '—' }}</p>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 uppercase mb-1">Subject</label>
                    <p class="text-sm text-gray-900">{{ $finding->subject?->name ?? '—' }}</p>
                </div>
            </div>
        </div>
