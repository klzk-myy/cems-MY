<x-app-layout title="Performance Monitoring">
        <header class="bg-white shadow-sm">
            <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                <h1 class="text-2xl font-semibold text-ink">Performance Monitoring</h1>
            </div>
        </header>
            ...
            <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                    <div class="bg-white border border-[#e5e5e5] rounded-xl p-6">
                        <h2 class="text-lg font-medium text-ink mb-4">Cache Statistics</h2>
                        <div class="space-y-4">
                    ...
                    <div class="bg-white border border-[#e5e5e5] rounded-xl p-6">
                        <h2 class="text-lg font-medium text-ink mb-4">Query Statistics</h2>
                        <div class="space-y-4">
                            <div class="flex justify-between items-center py-3 border-b border-gray-100">
                ...
                <div class="mt-6 bg-white border border-[#e5e5e5] rounded-xl p-6">
                    <h2 class="text-lg font-medium text-ink mb-4">System Health</h2>
                    <div class="grid grid-cols-4 gap-4">
                        <div class="text-center p-4 bg-canvas-subtle rounded-lg">
                            <p class="text-2xl font-semibold text-ink">{{ number_format($metrics['query_count']) }}</p>
                            <p class="text-sm text-ink-muted mt-1">Total Queries</p>
                        </div>
                        <div class="text-center p-4 bg-canvas-subtle rounded-lg">
                            <p class="text-2xl font-semibold {{ $metrics['slow_query_count'] > 0 ? 'text-yellow-600' : 'text-green-600' }}">{{ $metrics['slow_query_count'] }}</p>
                            <p class="text-sm text-ink-muted mt-1">Slow Queries</p>
                        </div>
                        <div class="text-center p-4 bg-canvas-subtle rounded-lg">
                            <p class="text-2xl font-semibold {{ $metrics['n_plus_one_count'] > 0 ? 'text-red-600' : 'text-green-600' }}">{{ $metrics['n_plus_one_count'] }}</p>
                            <p class="text-sm text-ink-muted mt-1">N+1 Queries</p>
                        </div>
                        <div class="text-center p-4 bg-canvas-subtle rounded-lg">
                            <p class="text-2xl font-semibold text-ink">{{ number_format($metrics['total_query_time_ms'], 0) }}ms</p>
                            <p class="text-sm text-ink-muted mt-1">Total Time</p>
                        </div>
                    </div>
                </div>
            </div>
