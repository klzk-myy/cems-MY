    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Report Date: {{ $date }}</p>
            </div>
            <a href="{{ url()->previous() }}"
        ...
        <div class="bg-white border border-[#e5e5e5] rounded-xl p-6 mb-6">
            <h2 class="text-lg font-medium text-gray-900 mb-4">Till Balance Details</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                    <dt class="text-sm font-medium text-gray-500">Till / Counter</dt>
                    <dd class="mt-1 text-sm text-gray-900">
                        {{ $balances->counter->name ?? 'N/A' }}
                    </dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">Opening Float</dt>
                    <dd class="mt-1 text-sm text-gray-900">
                        MYR {{ number_format((float) $balances->opening_float, 2) }}
                    </dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500">Closing Float</dt>
                    <dd class="mt-1 text-sm text-gray-900">
                ...
                <div>
                    <dt class="text-sm font-medium text-gray-500">Closing By</dt>
                    <dd class="mt-1 text-sm text-gray-900">
        ...
        <div class="bg-white border border-[#e5e5e5] rounded-xl p-6">
            <h2 class="text-lg font-medium text-gray-900 mb-4">Currency Balances</h2>
                ...
                <table class="min-w-full divide-y divide-[#e5e5e5]">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Opening Balance</th>
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Total Bought</th>
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Total Sold</th>
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Closing Balance</th>
                        ...
                        @forelse($balances->currencyBalances ?? [] as $currencyBalance)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-sm text-right">
                                    ...
                                    @endphp
                                    <span class="@if($variance != 0) text-red-600 font-medium @else text-gray-900 @endif">
