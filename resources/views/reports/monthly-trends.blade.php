<x-app-layout title="Monthly Transaction Trends">
    <div class="space-y-6">
        <x-page-header title="Monthly Transaction Trends" description="Analyze transaction volumes and counts by month">
            <x-slot:actions>
        ...
        @php
            ...
            $totalVolume = $monthlyData->sum(fn ($d) => (float) $d['volume']);
            $avgMonthlyVolume = $monthlyData->count() > 0 ? $totalVolume / $monthlyData->count() : 0;
        @endphp

        <x-stat-grid cols="3">
            <x-stat-card label="Total Transactions ({{ $year }})" color="blue">{{ number_format($totalTransactions) }}</x-stat-card>
            <x-stat-card label="Total Volume (MYR)" color="green">RM {{ number_format($totalVolume, 2) }}</x-stat-card>
            <x-stat-card label="Avg Monthly Volume" color="yellow">RM {{ number_format($avgMonthlyVolume, 2) }}</x-stat-card>
        </x-stat-grid>
