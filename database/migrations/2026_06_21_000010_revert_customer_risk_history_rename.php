...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasColumn('customer_risk_history', 'old_score')) {
            return;
        }
        ...
        if ($driver === 'sqlite') {
            ...
            ');
            DB::statement('
                INSERT INTO customer_risk_history (id, customer_id, previous_score, new_score, previous_rating, new_rating, change_reason, changed_by, changed_at, created_at, updated_at)
                SELECT id, customer_id, old_score, new_score, old_rating, new_rating, change_reason, assessed_by, NULL, created_at, updated_at
                FROM _customer_risk_history_old
            ');
            DB::statement('DROP TABLE _customer_risk_history_old');
        } else {
