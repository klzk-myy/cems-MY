...
class ComplianceReportingService
{
    ...
    protected function getRiskDistribution(): array
    {
        ...
        return [
            'Low' => $profiles[RiskRating::Low->value] ?? 0,
            'Medium' => $profiles[RiskRating::Medium->value] ?? 0,
            'High' => $profiles[RiskRating::High->value] ?? 0,
            'Critical' => $profiles[RiskRating::Critical->value] ?? 0,
        ];
    }
    ...
    public function getAutoGeneratedReports(): array
    {
        $reports = ReportGenerated::query()
            ->whereIn('report_type', [ReportType::Msb2->value, ReportType::Lmca->value, ReportType::Qlvr->value])
